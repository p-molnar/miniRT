...
/*   Updated: 2023/07/20 16:22:37 by pmolnar       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_mx apply_origin_transformations(t_mx origin, t_scn_el *obj_info)
{
	...
	// print_mx(&obj_info->inv_translation);
...
t_ray	apply_transformations(t_ray ray, t_scn_el *obj_info)
{
	...
	origin_mx = apply_origin_transformations(coord_to_mx(ray.origin, 4, 1, 1), obj_info);
	transformed_ray.origin = create_coord(origin_mx.m[0], origin_mx.m[1], origin_mx.m[2]);
	dir_mx = apply_direction_transformations(coord_to_mx(ray.dir.dir, 4, 1, 1), obj_info);
