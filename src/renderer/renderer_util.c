...
t_vec3	get_cylinder_norm(t_hit_obj obj)
{
	...
	if ((float)obj.inc_p.z == (float)obj.attr->cap[0].pos.z)
		obj_norm = obj.attr->cap[0].n_vec;
	else if ((float)obj.inc_p.z == (float)obj.attr->cap[1].pos.z)
		obj_norm = obj.attr->cap[1].n_vec;
	else
		obj_norm = create_dir_vec((t_coord3){{0, 0, obj.inc_p.z}}, obj.inc_p);
	obj_norm_mx = multiply_mx(obj.attr->rotation, coord_to_mx(obj_norm.dir, 4, 1,
				1));
...
t_vec3	get_obj_norm(t_scn_el cam, t_coord3 inc_p, t_hit_obj obj)
{
	...
	if (obj.attr->type == F_CYLINDER)
		obj_norm = get_cylinder_norm(obj);
	else if (obj.attr->type == F_SPHERE)
		obj_norm = create_dir_vec(obj.attr->pos, inc_p);
	else if (obj.attr->type == F_PLANE)
	{
		agl = get_agl_between(obj.attr->n_vec, cam.n_vec);
		obj_norm = coord_to_vec(obj.attr->n_vec.dir);
		if (obj_norm.dir.x)
...
t_coord3	get_incident_point(t_ray ray, t_hit_obj obj)
{
	...
	if (obj.attr->type == F_CYLINDER)
	{
		...
		inc_p_mx = multiply_mx(obj.attr->rotation, inc_p_mx);
		inc_p_mx = multiply_mx(obj.attr->translation, inc_p_mx);
